...
exports.QuickPromptsProvider = exports.DEFAULT_PROMPTS = exports.STORAGE_KEY = exports.CONFIG_KEY = void 0;
exports.getPositionConfig = getPositionConfig;
const vscode = __importStar(require("vscode"));
exports.CONFIG_KEY = "copilotQuickPrompts.statusBarPosition";
/**
 * 根据配置获取状态栏对齐方式和优先级
...
function getPositionConfig() {
    const position = vscode.workspace
        .getConfiguration()
        .get(exports.CONFIG_KEY, "leftRight");
    switch (position) {
        case "leftLeft":
...
exports.STORAGE_KEY = "copilotQuickPrompts.prompts";
/** 默认预设提示词列表 */
...
class QuickPromptsProvider {
    ...
    /** 从全局存储加载提示词 */
    loadPrompts() {
        const saved = this.storage.get(exports.STORAGE_KEY);
        if (saved) {
            try {
        ...
        return exports.DEFAULT_PROMPTS.map((p) => ({ ...p }));
    }
    /** 保存提示词到全局存储 */
    savePrompts() {
        this.storage.update(exports.STORAGE_KEY, JSON.stringify(this.prompts));
    }
    /** 向 webview 发送更新后的数据 */
    postState() {
        const position = vscode.workspace
    ...
    resolveWebviewView(webviewView, _context, _token) {
        ...
        this._configListener = vscode.workspace.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration(exports.CONFIG_KEY) && this.webviewView) {
                this.postState();
            }
        });
        // 向 webview 发送初始数据
        webviewView.webview.onDidReceiveMessage(async (message) => {
            switch (message.type) {
                ...
                case "sendPromptWithEditor":
                    await this.handleSendWithEditor(message.prompt, message.mode ?? "write");
                    break;
                case "savePrompts":
                ...
                case "updatePosition": {
                    await vscode.workspace
                        .getConfiguration()
                        .update(exports.CONFIG_KEY, message.position, vscode.ConfigurationTarget.Global);
                    break;
                }
            }
        });
    ...
    async sendPrompt(prompt, mode) {
        await vscode.commands.executeCommand("copilotQuickPrompts.sendPrompt", prompt, mode);
    }
    /** 发送提示词并附加上当前编辑器选中的代码 */
    async handleSendWithEditor(prompt, mode) {
        ...
        await vscode.commands.executeCommand("copilotQuickPrompts.sendPrompt", fullPrompt, mode);
    }
    getHtmlContent() {
        return `<!DOCTYPE html>
<html lang="zh-CN">
...
exports.QuickPromptsProvider = QuickPromptsProvider;
//# sourceMappingURL=quickPromptsProvider.js.map